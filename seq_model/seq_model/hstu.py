...
class Hstu(nn.Module):
    ...
    def log2feats(self, log_seqs):
        ...
        # 创建causal mask
        invalid_attn_mask = torch.tril(torch.ones(seq_len, seq_len, device=self.dev))
    ...
    def forward(self, user_ids, log_seqs, pos_seqs, neg_seqs):
        ...
        pos_embs = self.item_embedding(torch.LongTensor(pos_seqs).to(self.dev))
        neg_embs = self.item_embedding(torch.LongTensor(neg_seqs).to(self.dev))
        # get logits
    ...
    def predict(self, user_ids, log_seqs, item_indices):
        ...
        item_embs = self.item_embedding(torch.LongTensor(item_indices).to(self.dev))
